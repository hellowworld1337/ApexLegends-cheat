...
namespace RenderToolKit
{
	...
	//Render Context
	#pragma bss_seg(push, ".rctx")
	unsigned char RenderBuff[0x80000];
	#pragma bss_seg(pop)
...
#include "RenderData.h"
	...
	*DrawList = (DrawListStruct*)&RenderBuff[0];
	...
	__forceinline T DeEncPtr(IDXGISwapChain* SwapChain, T Ptr) {
		return T((DWORD64)Ptr ^ ((DWORD64)SwapChain + 0xDEADBEEF));
	}

	__forceinline void SafeDecrShader(PUCHAR BuffIn, PUCHAR BuffOut, int Size) {
		for (int i = 0; i < Size; ++i)
			BuffOut[i] = (UCHAR)(BuffIn[i] ^ ((i + 13 * i + 57) + 76 + i));
		ZeroSet(BuffIn, Size);
	}
	...
	public:
		void SaveState(IDXGISwapChain* SwapChain, ID3D11DeviceContext* Context)
		{
			...
			VCall(Context, 95, &ViewPorts_Num, ViewPorts);
		...
		void LoadState(IDXGISwapChain* SwapChain, ID3D11DeviceContext* Context)
		{
			...
			VCall(Context, 44, ViewPorts_Num, ViewPorts);
			...
			VCall(Context, 35, DeEncPtr(SwapChain, TransparencyState), BlendFactor, SampleMask);
	...
	{
		int CharID = (int)Char;
		CharDesc* FDBuff = (CharDesc*)&FontDescBytesName;
		...
		else if ((CharID >= 218) && (CharID <= 218)) {
			*Desc = FDBuff[95 + (CharID - 218)];
			return true;
		}

		else if ((CharID >= 1025) && (CharID <= 1025)) {
			*Desc = FDBuff[95 + (CharID - 1025)];
			return true;
		}

		else if ((CharID >= 1040) && (CharID <= 1103)) {
			*Desc = FDBuff[96 + (CharID - 1040)];
			return true;
		}

		else if ((CharID >= 1105) && (CharID <= 1105)) {
	...
	__forceinline bool InitFontTexture(IDXGISwapChain* SwapChain, ID3D11Device* Device)
	{
		...
		//cleanup font bytes
		ZeroSet(FontBytesName, sizeof(FontBytesName));
	...
	Vector2 GetTextSize(const wchar_t* String, RenderToolKit::CharDesc* DescArray, int* DescSize, float MP)
	{
		wchar_t WChar;
		...
		for (int i = 0; (WChar = String[i]); ++i)
		{
			...
			if (GetCharDesc(WChar, &DescArray[DescArrayOffset]))
			{
				...
				float YSize = float(CDesc.yOff + CDesc.srcH) * MP;
				TextSize.x += ((float)CDesc.xAdv * MP);
	...
	//D3D11 Render
	bool InitRender(IDXGISwapChain* SwapChain, bool ForceScreenSize)
	{
		//get d3d device
		ID3D11Device* D3Device;
		if (FAILED(VCall<HRESULT>(SwapChain, 7, IID_PPV_ARGS(&D3Device))))
			return false;
		...
		if (FAILED(VCall<HRESULT>(D3Device, 3, &VBufferDesc, nullptr, &VBufferDecr))) {
			VCall(PShaderDecr, 2); VCall(VShaderDecr, 2);
			VCall(RState, 2); VCall(TState, 2); return false;
	...
	bool NewFrame(IDXGISwapChain* SwapChain, Vector2& ScreenSize, bool ForceScreenSize = false)
	{
		//init d3d11
		static bool Once = false;
		if (!SwapChain) return false;
		if (!Once) {
			if (!InitRender(SwapChain, ForceScreenSize))
				return false;
			Once = true;
		}
		...
		D3DState.SaveState(SwapChain, Ctx);

		//get viewport params
		if (!ForceScreenSize) {
			VCall(Ctx, 95, &Number, &ViewPort);
		}
...
namespace Render
{
	...
	__forceinline void Rectangle(const Vector2& Start, const Vector2& Size, const Color& Color, float Thickness = 1.f)
	{
		...
		RenderToolKit::AddPolyLineAA(Pnts, 4, Color.DXColor, true, Thickness);
	}
	...
	//render text
	Vector2 String(Vector2 Start, const wchar_t* String, bool Center = false, const Color& Color = Render::Color(255, 255, 255), float MP = 0.5f)
	{
		...
		Vector2 TextSize = GetTextSize(String, CharDesc, &CharDescSize, MP);
